...
export const COMMAND_DOCS: CommandDoc[] = [
  ...
  { name: '/salute', group: 'Utilità', permission: 'Nessuno', summary: 'Dice quale pezzo non funziona: database, Redis, disco, versione, latenza.', caution: "Serve quando i comandi smettono di rispondere: da Discord si vedrebbe solo «l'applicazione non ha risposto», che è vero per qualsiasi causa." },
  { name: '/segnala', group: 'Utilità', permission: 'Nessuno', summary: 'Segnala una persona allo staff: arriva nel canale riservato con i pulsanti per intervenire.', example: '/segnala utente:@tizio motivo:insulti ripetuti in chat', caution: 'Il nome di chi segnala è visibile solo allo staff. Le prove restano anche se il messaggio viene cancellato subito dopo.' },
  { name: '/azioni', group: 'Moderazione', permission: 'Moderare membri', summary: 'Apre i pulsanti rapidi — silenzia, quarantena, espelli, bandisci — su una persona.', example: '/azioni utente:@tizio', caution: 'I pulsanti passano dagli stessi controlli dei comandi: permessi, gerarchia dei ruoli, provvedimento registrato.' },
  ...
  { name: '/crea-server', group: 'Utilità', permission: 'Amministratore', summary: 'Costruisce il server: ruoli, categorie, canali, modalità community, verifica e ticket.', example: '/crea-server', caution: 'Si può rieseguire quando si vuole: cerca ogni cosa per nome prima di crearla, e non sovrascrive i campi già compilati. Il bot non può creare il server: creandolo lui ne resterebbe proprietario, e la proprieta non e trasferibile.' },
